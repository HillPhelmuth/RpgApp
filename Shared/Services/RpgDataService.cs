...
namespace RpgApp.Shared.Services
{
    public class RpgDataService
    {
        ...
        public async Task<List<Player>> GetUserPlayers(string userId)
        {
            ...
            var playersFromDb = await _context.Players.Where(x => x.UserId == userId).Include(x => x.Inventory).ThenInclude(y => y.Equipment).ThenInclude(z => z.Effects).Include(x => x.Skills).ThenInclude(y => y.Skill).ThenInclude(z => z.Effects).ToListAsync();
